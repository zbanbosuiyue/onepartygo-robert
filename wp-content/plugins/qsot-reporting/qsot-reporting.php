...
/**
 * Plugin Name: OpenTickets - Advanced Reporting
 * Plugin URI:  http://opentickets.com/software/opentickets-advanced-reporting/
 * Description: Adds several useful reports for tracking ticket sales.
 * Version:     1.0.1
 * Author:      Quadshot Software LLC
 * Author URI:  http://quadshot.com/
 * Text Domain: qsot-reporting
 * Domain Path: /langs/
...
// checks all reporting extension dependencies and sets up the plugin in general
class QSOT_reporting_launcher {
	// urls to relevant quadshot products
	protected static $urls = array(
		'me' => 'http://opentickets.com/product/opentickets-reporting/',
	...
	protected static $version_key = 'qsot_reporting_version';
	// plugin paths
	...
	protected static $in_conflict_key = 'qsot_reporting_in_conflict';
	protected static $active = false;
	// min versions of required softwares
	protected static $min_wc_version = '2.4.12';
	...
	public static function pre_init() {
		...
		self::$name = __( 'OpenTickets - Reporting', 'qsot-reporting' );
	...
	public static function load_text_domain() {
		load_plugin_textdomain( 'qsot-reporting', false, dirname( self::$me ) . '/langs/' );
	}
	...
	protected static function _pass_system_requirements() {
		...
		if ( ! self::_is_opentickets_active() ) {
			...
			$file = substr( self::$me, 0, strpos( self::$me, 'qsot-reporting' ) ) . implode( DIRECTORY_SEPARATOR, array( 'opentickets-community-edition', 'launcher.php' ) );
		...
		if ( ! self::_is_woocommerce_active() ) {
			...
			$file = substr( self::$me, 0, strpos( self::$me, 'qsot-reporting' ) ) . implode( DIRECTORY_SEPARATOR, array( 'woocommerce', 'woocommerce.php' ) );
			add_action( 'activate_' . $file, array( __CLASS__, 'activation' ), 1000001 );
			$pass = false;
	...
	public static function inform_core_about_us( $plugin_directories ) {
		...
		array_unshift( $plugin_directories, self::$plugin_dir . 'reporting/' );
		...
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( '<u><strong>%s</strong></u><br/> The %s plugin <strong>requires</strong> that %s be at least at version %s. You currently have version %s. Please upgrade %s to finish activating all functionality of the %s plugin.', 'qsot-reporting' ),
					self::_me_link(),
					self::_me_link(),
					$dep,
					$min_version,
	...
	protected static function _requires_plugin( $req_plugin ) {
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( '<u><strong>%s</strong></u><br/> The %s plugin <strong>requires</strong> that %s be activated in order to perform most vital functions; therefore, the plugin has not initialized any of its functionality. To enable the features of this plugin, simply install and activate %s.', 'qsot-reporting' ),
					self::_me_link(),
					self::_me_link(),
					$req_plugin,
					$req_plugin
	...
	public static function error_conflicting_plugins_active() {
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( 'There are one or more plugins active that have know conflicts with %s. Please deactivate the following plugins, in order to ensure that the %s plugin functions properly: <strong><em>%s</em></strong>', 'qsot-reporting' ),
					self::_me_link(),
					self::_me_link(),
	...
	public static function _has_conflicting_plugins() {
		...
		$conflicts = array(
			//'#^qsot-ga-multi-price(-(master|[\d\.]+(-(alpha|beta|RC\d+)(-\d+)?)?))?[\/\\\\]qsot-ga-multi-price.php$#' => __( 'OpenTickets General Admission Multiprice', 'qsot-reporting' ),
		);
	...
	public static function activation() {
		...
		$path = QSOT_reporting_launcher::plugin_dir() . 'reporting/';
		...
		do_action( 'qsot-reporting-activation' );
	}
...
if ( defined( 'ABSPATH' ) && function_exists( 'add_action' ) ) {
	QSOT_reporting_launcher::pre_init();
}
