...
/**
 * Plugin Name: OpenTickets - Box Office
 * Plugin URI:  http://opentickets.com/product/opentickets-box-office/
 * Description: A set of tools to allow a box office to sell tickets, take payments, keep track of accountability, and various other administrative tasks.
 * Version:    	2.0.3
 * Author:      Quadshot Software LLC
 * Author URI:  http://quadshot.com/
 * Text Domain: qsot-box-office
 * Domain Path: /langs/
...
// checks all box-office plugin dependencies and sets up the plugin in general
class QSOT_box_office_launcher {
	// urls to relevant quadshot products
	protected static $urls = array(
		'me' => 'http://opentickets.com/product/opentickets-box-office/',
	...
	protected static $version_key = 'qsot_box_office_version';
	// plugin paths
	...
	protected static $in_conflict_key = 'qsot_box_office_in_conflict';
	protected static $active = false;
	// min versions of required softwares
	protected static $min_wc_version = '2.4.10';
	...
	public static function load_text_domain() {
		load_plugin_textdomain( 'qsot-box-office', false, dirname( self::$me ) . '/langs/' );
	}
	...
	protected static function _pass_system_requirements() {
		...
		if ( ! self::_is_opentickets_active() ) {
			...
			$file = substr( self::$me, 0, strpos( self::$me, 'qsot-box-office' ) ) . implode( DIRECTORY_SEPARATOR, array( 'opentickets-community-edition', 'launcher.php' ) );
			add_action( 'activate_' . $file, array( __CLASS__, 'activation' ), 1000002 );
			$pass = false;
		...
		if ( ! self::_is_woocommerce_active() ) {
			...
			$file = substr( self::$me, 0, strpos( self::$me, 'qsot-box-office' ) ) . implode( DIRECTORY_SEPARATOR, array( 'woocommerce', 'woocommerce.php' ) );
			add_action( 'activate_' . $file, array( __CLASS__, 'activation' ), 1000002 );
			$pass = false;
	...
	public static function inform_core_about_us( $plugin_directories ) {
		...
		array_unshift( $plugin_directories, trailingslashit( dirname( __FILE__ ) ) . 'box-office/' );
		...
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( '<u><strong>%s</strong></u><br/> The %s plugin <strong>requires</strong> that %s be at least at version %s. You currently have version %s. Please upgrade %s to finish activating all functionality of the %s plugin.', 'qsot-box-office' ),
					self::_me_link(),
					self::_me_link(),
					$dep,
					$min_version,
	...
	protected static function _requires_plugin( $req_plugin ) {
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( '<u><strong>%s</strong></u><br/> The %s plugin <strong>requires</strong> that %s be activated in order to perform most vital functions; therefore, the plugin has not initialized any of its functionality. To enable the features of this plugin, simply install and activate %s.', 'qsot-box-office' ),
					self::_me_link(),
					self::_me_link(),
					$req_plugin,
					$req_plugin
	...
	public static function error_conflicting_plugins_active() {
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( 'There are one or more plugins active that have know conflicts with %s. Please deactivate the following plugins, in order to ensure that the %s plugin functions properly: <strong><em>%s</em></strong>', 'qsot-box-office' ),
					self::_me_link(),
					self::_me_link(),
	...
	public static function _has_conflicting_plugins() {
		...
		$conflicts = array(
			//'#^qsot-ga-multi-price(-(master|[\d\.]+(-(alpha|beta|RC\d+)(-\d+)?)?))?[\/\\\\]qsot-ga-multi-price.php$#' => __( 'OpenTickets General Admission Multiprice', 'qsot-box-office' ),
		);
	...
	protected static function _me_link($link='', $label='', $format='') {
		...
		$label = $label ? $label : 'OpenTickets - Box-Office';
	...
	public static function activation() {
		...
		$path = QSOT_box_office_launcher::plugin_dir() . 'box-office/';
		...
		do_action('qsot-box-office-activation');
	}
...
if ( defined( 'ABSPATH' ) && function_exists( 'add_action' ) ) {
	QSOT_box_office_launcher::pre_init();
}
