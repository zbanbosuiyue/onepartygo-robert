...
class qsot_inhouse_cash extends WC_Payment_Gateway {
	public static function pre_init() {
		// add a function to register this payment gateway
	...
	public function __construct() { 
		$this->id = 'admin_only_cash';
		$this->method_title = __( 'Inhouse Cash', 'qsot-box-office' );        
		$this->icon = '';
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'description' => array(
				...
				'default' => __( 'End user comes in the box office with a cash payment.', 'qsot-box-office' ),
			),  
		);
	}
	...
	**/
	public function admin_options() {
		?>
			...
			<p><?php _e( 'Allows admin users to accept cash payments via the inhouse box office.', 'qsot-box-office' ) ?></p>
	...
	**/
	public function process_payment( $order_id ) {
		...
		// if the current user is allowed to use this method
		if ( is_admin() && current_user_can( 'edit_users' ) ) {
			// add a note indicating who used it
			$admin_user = wp_get_current_user();
			$order->add_order_note( $this->method_title . __( ' - payment complete | made by user: ', 'qsot' ) . $admin_user->user_login );

			// complete the order and notify other plugins
