...
/**
 * Plugin Name: OpenTickets - General Admission Multiple Pricing (GAMP)
 * Plugin URI:  http://opentickets.com/
 * Description: Provides the ability to assign multiple General Admission prices to an Event Area. 
 * Version:     2.1.2
 * Author:      Quadshot Software LLC
 * Author URI:  http://quadshot.com/
 * Text Domain: qsot-ga-multi-price
 * Domain Path: /langs/
...
// checks all GAMP plugin dependencies and sets up the plugin in general
class QSOT_Multi_Price_Launcher {
	// urls to relevant quadshot products
	protected static $urls = array(
		'me' => 'http://opentickets.com/software/opentickets-general-admission-multiple-pricing-gamp/',
	...
	protected static $version_key = 'qsot_gamp_version';
	// plugin paths
	...
	protected static $in_conflict_key = 'qsot_gamp_in_conflict';
	...
	public static function pre_init() {
		...
		self::$name = __( 'OpenTickets - General Admission Multiple Pricing (GAMP)', 'qsot-ga-multi-price' );
	...
	public static function load_text_domain() {
		load_plugin_textdomain( 'qsot-ga-multi-price', false, dirname( self::$me ) . '/langs/' );
	}
	...
	protected static function _pass_system_requirements() {
		...
		if ( ! self::_is_opentickets_active() ) {
			...
			$file = substr( self::$me, 0, strpos( self::$me, 'qsot-ga-multi-price' ) ) . implode( DIRECTORY_SEPARATOR, array( 'opentickets-community-edition', 'launcher.php' ) );
		...
		if ( ! self::_is_woocommerce_active() ) {
			...
			$file = substr( self::$me, 0, strpos( self::$me, 'qsot-ga-multi-price' ) ) . implode( DIRECTORY_SEPARATOR, array( 'woocommerce', 'woocommerce.php' ) );
			add_action( 'activate_' . $file, array( __CLASS__, 'activation' ), 1000001 );
			$pass = false;
	...
	public static function inform_core_about_us( $plugin_directories ) {
		...
		array_unshift( $plugin_directories, self::$plugin_dir . 'multi-price/' );
		...
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( '%s The %s plugin <strong>requires</strong> that %s be at least at version %s. You currently have version %s. Please upgrade %s to finish activating all functionality of the %s plugin.', 'qsot-ga-multi-price' ),
					'<u><strong>' . self::_me_link() . '</strong></u><br/>',
					self::_me_link(),
					$dep,
					$min_version,
					$cur_version,
	...
	protected static function _requires_plugin( $req_plugin ) {
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( '%s The %s plugin <strong>requires</strong> that %s be activated in order to perform most vital functions; therefore, the plugin has not initialized any of its functionality. To enable the features of this plugin, simply install and activate %s.', 'qsot-ga-multi-price' ),
					'<u><strong>' . self::_me_link() . '</strong></u><br/>',
					self::_me_link(),
					$req_plugin,
					$req_plugin
	...
	public static function error_conflicting_plugins_active() {
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( 'There are one or more plugins active that have know conflicts with %s. Please deactivate the following plugins, in order to ensure that the %s plugin functions properly: <strong><em>%s</em></strong>', 'qsot-ga-multi-price' ),
					self::_me_link(),
					self::_me_link(),
	...
	public static function activation() {
		...
		$path = QSOT_Multi_Price_Launcher::plugin_dir() . 'multi-price/';
		require_once $path . 'price-struct.class.php';
		require_once $path . 'gamp-area-type.class.php';

		// possibly trigger the db upgrader
		...
		do_action( 'qsot-gamp-activation' ); // deprecated in a later version
		do_action( 'qsot-ga-multi-price-activation' );
	}
...
if ( defined( 'ABSPATH' ) && function_exists( 'add_action' ) ) {
	QSOT_Multi_Price_Launcher::pre_init();
}
