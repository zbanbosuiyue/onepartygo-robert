...
class qsot_inhouse_other extends WC_Payment_Gateway {
	...
	public function __construct() { 
		$this->id = 'inhouse_other';
		$this->method_title = __( 'Inhouse Other', 'qsot-box-office' );        
		$this->icon = '';
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'description' => array(
				...
				'default' => __( 'End user comes in the box office with a payment that is not covered by any other payment types.', 'qsot-box-office' )
			)
		);
	}
	...
	public function payment_fields() {
		...
		<fieldset>
			...
			<p class="form-row form-row-first">
				<label for="other-info"><?php echo __( 'Payment Info', 'qsot-box-office' ) ?> <span class="required">*</span></label>
				<input type="text" class="input-text" id="other-info" name="other-info" />
			</p>
	...
	**/
	public function admin_options() {
		?>
			...
			<p><?php _e( 'Allows admin users to accept payments, not covered by other payment types, via the inhouse box office.', 'qsot-box-office' ) ?></p>
	...
	**/
	...
	public function validate_fields() {
		...
		$other_info = trim($this->get_post('other-info'));
		if (empty($other_info)) {
			$woocommerce->add_error(sprintf(__('You must fill out the payment info field. %s Payment Failed.', 'qsot-box-office'), $this->method_title));
			return false;
		}
