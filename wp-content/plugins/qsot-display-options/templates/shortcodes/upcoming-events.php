	...
	<?php if ( have_posts() ): while ( have_posts() ): the_post(); $product = wc_get_product( $post->ID ); ?>
		...
		<li <?php post_class( 'list-item event-item' ) ?>>
			...
			<?php if ( $show_image && has_post_thumbnail() ): ?>
				<div class="event-thumb"><a href="<?php echo esc_attr( esc_url( get_permalink() ) ) ?>" rel="bookmark"><?php the_post_thumbnail( 'thumbnail' ) ?></a></div>
			<?php endif; ?>
							...
							switch ( $field ) {
								case 'date':
									?><div class="item-date"><?php
										...
										$start = date_i18n( $date_format, $start, true );
										$end = date_i18n( $same_day ? $time_format : $date_format, $end, true );
