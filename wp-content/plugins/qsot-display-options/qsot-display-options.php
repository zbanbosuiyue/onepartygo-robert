...
/**
 * Plugin Name: OpenTickets - Display Options
 * Plugin URI:  http://opentickets.com/product/display-options/
 * Description: Adds smart widgets for displaying the different elements of OpenTickets. Also provides different options for displaying your events on the frontend.
 * Version:     2.1.0
 * Author:      Quadshot Software LLC
 * Author URI:  http://quadshot.com/
 * Text Domain: qsot-display-options
 * Domain Path: /langs/
...
// checks all display options plugin dependencies and sets up the plugin in general
class QSOT_display_options_launcher {
	// urls to relevant quadshot products
	protected static $urls = array(
		'me' => 'http://opentickets.com/product/opentickets-display-options/',
	...
	protected static $version_key = 'qsot_display_options_version';
	// plugin paths
	...
	protected static $in_conflict_key = 'qsot_display_options_in_conflict';
	protected static $active = false;
	// min versions of required softwares
	protected static $min_wc_version = '2.4.5';
	protected static $min_otce_version = '2.4.0';
	...
	public static function pre_init() {
		...
		self::$name = __( 'OpenTickets - Display Options', 'qsot-display-options' );
	...
	public static function load_text_domain() {
		load_plugin_textdomain( 'qsot-display-options', false, dirname( self::$me ) . '/langs/' );
	}
	...
	protected static function _pass_system_requirements() {
		...
		if ( ! self::_is_opentickets_active() ) {
			...
			$file = substr( self::$me, 0, strpos( self::$me, 'qsot-display-options' ) ) . implode( DIRECTORY_SEPARATOR, array( 'opentickets-community-edition', 'launcher.php' ) );
		...
		if ( ! self::_is_woocommerce_active() ) {
			...
			$file = substr( self::$me, 0, strpos( self::$me, 'qsot-display-options' ) ) . implode( DIRECTORY_SEPARATOR, array( 'woocommerce', 'woocommerce.php' ) );
			add_action( 'activate_' . $file, array( __CLASS__, 'activation' ), 1000001 );
			$pass = false;
	...
	public static function inform_core_about_us( $plugin_directories ) {
		...
		array_unshift( $plugin_directories, self::$plugin_dir . 'display-options/' );
	...
	public static function add_template_directory( $list, $qsot_path='', $woo_path='', $type=false ) {
		...
		*/
		array_unshift( $list, self::$plugin_dir . 'templates/' );
		...
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( '<u><strong>%s</strong></u><br/> The %s plugin <strong>requires</strong> that %s be at least at version %s. You currently have version %s. Please upgrade %s to finish activating all functionality of the %s plugin.', 'qsot-display-options' ),
					self::_me_link(),
					self::_me_link(),
					$dep,
					$min_version,
	...
	protected static function _requires_plugin( $req_plugin ) {
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( '<u><strong>%s</strong></u><br/> The %s plugin <strong>requires</strong> that %s be activated in order to perform most vital functions; therefore, the plugin has not initialized any of its functionality. To enable the features of this plugin, simply install and activate %s.', 'qsot-display-options' ),
					self::_me_link(),
					self::_me_link(),
					$req_plugin,
					$req_plugin
	...
	public static function error_conflicting_plugins_active() {
		?>
			<div class="error errors">
				<p class="error"><?php echo sprintf(
					__( 'There are one or more plugins active that have know conflicts with %s. Please deactivate the following plugins, in order to ensure that the %s plugin functions properly: <strong><em>%s</em></strong>', 'qsot-display-options' ),
					self::_me_link(),
					self::_me_link(),
	...
	public static function _has_conflicting_plugins() {
		...
		$conflicts = array(
			//'#^qsot-ga-multi-price(-(master|[\d\.]+(-(alpha|beta|RC\d+)(-\d+)?)?))?[\/\\\\]qsot-ga-multi-price.php$#' => __( 'OpenTickets General Admission Multiprice', 'qsot-display-options' ),
		);
	...
	protected static function _me_link($link='', $label='', $format='') {
		...
		$label = $label ? $label : __( 'OpenTickets - Display Options', 'qsot-display-options' );
	...
	public static function activation() {
		...
		$path = QSOT_display_options_launcher::plugin_dir() . 'display-options/';
		require_once $path . 'core.class.php';
		require_once $path . 'admin.class.php';

		// let child files and other plugins know this specific activation is happening
		do_action( 'qsot-display-options-activation' );
	}
...
if ( defined( 'ABSPATH' ) && function_exists( 'add_action' ) ) {
	QSOT_display_options_launcher::pre_init();
}
