...
if ( ! class_exists( 'QSOT_All_Product_Sales_Report' ) ):
...
class QSOT_All_Product_Sales_Report extends QSOT_Admin_Report {
	...
	public function init() {
		...
		$this->group_name = $this->name = __( 'All Product Sales', 'qsot-reporting' );
		$this->group_slug = $this->slug = 'all-sales';
	...
	// augment the clean memory procedure to clear out cached products too
	protected function _clean_memory() {
		foreach ( $this->products as $ind => $product )
			unset( $this->products[ $ind ] );
		unset( $this->products );
		$this->products = array();
		parent::_clean_memory();
	}
}

endif;

return new QSOT_All_Product_Sales_Report();
