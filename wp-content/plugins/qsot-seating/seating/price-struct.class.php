...
class QSOT_Seating_Price_Struct {
	...
	public static function instance() {
		...
		if ( isset( self::$instance ) && self::$instance instanceof QSOT_Seating_Price_Struct )
			return self::$instance;

		// otherwise, start a new instance
		return self::$instance = new QSOT_Seating_Price_Struct();
	}
	...
	public function initialize() {
		$this->cacher = QSOT_Seating_Cacher::instance();
	...
	public function get_by_event_area_id( $event_area_id, $extra_params='', $force=false ) {
		$extra_params = wp_parse_args( $extra_params, array() );
		// get all the matching structs
		$result = $this->find( wp_parse_args( array( 'event_area_id' => $event_area_id ), $extra_params ), $force );
	...
	public function find( $args='', $force=false ) {
		// normalize the input
		...
		$ids = ! $force ? $this->cacher->get( $key, 'ps-find', false, $found ) : false;
		...
		if ( ( null !== $found && ! $found ) || ( null === $found && false === $ids ) ) {
			...
			// update the cache
			$this->cacher->set( $key, $ids, 'ps-find', 3600 );
		}
		...
		foreach ( $ids as $id ) {
			...
			$cache = $this->cacher->get( $key, $group, false, $found );
		...
		if ( ! empty( $lookup ) ) {
			...
			if ( is_array( $raw_structs ) && ! empty( $raw_structs ) ) {
				...
				while ( count( $structs ) ) {
					...
					$this->cacher->set( 'price-struct-' . $struct->id, $struct, 'ea-structs-' . $struct->event_area_id, 3600 );
	...
	public function update_structs( $updates ) {
		...
		foreach ( $updates as $update ) {
			foreach ( $update['new']->prices as $price ) {
				...
				$next = ',';
				$cache_clear_pairs[] = array( 'price-struct-' . $update['old']->id, 'ea-structs-' . $update['old']->event_area_id );
			}
		}
		...
		if ( $next ) {
			$wpdb->query( $q );
			$this->cacher->delete( $cache_clear_pairs );
		}
	}
	...
	// accepts data sent on the save action for the seating chart, and translates that into actual pricing data that can be used to update the pricing structurs for the chart
	public function consolidate_pricing( $pricing, &$zones, $new_zone_map ) {
		$out = $pricing;

		// convert all zone pricing to objects out of the json strings we received
		...
		foreach ( $out as $struct_id => &$struct ) {
			// make sure the struct array is in the right format, with a sub array for each sub-group, starting with the 0 sub-group (0 = global seating chart pricing)
			if ( ! isset( $struct['prices'] ) || ! isset( $struct['prices'][0] ) || ! is_array( $struct['prices'][0] ) )
				$struct['prices'] = array( '0' => $struct['prices'] );

			// cycle through the zones. if any have specialized pricing, update the struct prices with a new sub-group for that zone
			foreach ( $zones as $zone_id => &$zone ) {
				// if this zone does not have special pricing, skip it
				if ( ! isset( $zone->meta, $zone->meta['pricing'] ) )
					continue;

				// store the pricing for this zone
				$pricing = $zone->meta['pricing'];

				// look up the actual zone id. this could be a new zone, which started with a negative id (like -1) and got mapped to a new id (like zone_id 123). thus we may need to look it up
				$z_id = $zone_id > 0 ? $zone_id : ( isset( $new_zone_map[ $zone_id ] ) ? $new_zone_map[ $zone_id ] : false );

				// if there was not a proper zone_id, then skip this zone too
				if ( ! $z_id )
					continue;

				// check if the specialized pricing for this zone defines specialized pricing for this struct. if so, update this struct's pricing with a new sub-group for the zone
	...
	// update the pricing structures of the seating chart
	public function save_pricing( $pricing, $seating_chart_id ) {
		global $wpdb;

		// get the current pricing fr the chart
		$current = $this->get_by_event_area_id( $seating_chart_id, array( 'price_list_format' => 'ids', 'price_sub_group' => '' ) );
		
		// figure out if any pricing structures were deleted from the new list, compared to the old list
		$deleted = array_diff( array_keys( $current ), array_keys( $pricing ) );

		// if any were removed, then remove the prices for that structure first, then remove the structure itself
		if ( count( $deleted ) ) {
			...
			foreach ( $deleted as $did )
				$delete_keys[] = array( 'price-struct-' . $did, 'ea-structs-' . $seating_chart_id );
		}

		// figure out the maximum sized query we can send, and set our internal max for this function to 75% of that number
		$max_packet = round( 0.75 * QSOT::max_packet() );

		// create one master insert query, which will be used to insert all of the new prices for all the supplied pricing structs
		$qcnt = 0;
		$base_iq = $iq = 'insert into '. $wpdb->qsot_price_struct_prices . ' ( price_struct_id, product_id, display_order, sub_group ) values ';
		$iq_len = strlen( $iq );
		$next_q = '';

		// cycle through the new list of prices, and perform the various inserts, updates, and removals needed
		if ( is_array( $pricing ) ) foreach ( $pricing as $struct_id => $struct ) {
			// if the supplied struct id is less than 0, then the structure does not exist yet. therefore we need to create it
			if ( $struct_id < 0 ) {
				$wpdb->insert( $wpdb->qsot_price_structs, array( 'event_area_id' => $seating_chart_id, 'name' => $struct['name'] ) );
				$struct_id = $wpdb->insert_id;
			// otherwise, just update the struct
			} else {
				$delete_keys[] = array( 'price-struct-' . $struct['id'], 'ea-structs-' . $seating_chart_id );
				$wpdb->update( $wpdb->qsot_price_structs, array( 'event_area_id' => $seating_chart_id, 'name' => $struct['name'] ), array( 'id' => $struct['id'] ) );
			}

			// if there is still not a valid struct id, then bail
			if ( $struct_id <= 0 )
				continue;

			// we need to update the entire list of prices for this structure. this could be a series of inserts, updates and deletes... however, for simplicity, we just remove them all, and re-insert them all,
			// thereby inserting any new ones, updating an existing ones accordingly, and removing any that need removed

			// remove all the old pricing entries for this structure
			$q = $wpdb->prepare( 'delete from ' . $wpdb->qsot_price_struct_prices . ' where price_struct_id = %d', $struct_id );
			$wpdb->query( $q );

			// cycle through all the prices for this struct, which are broken down into pricing groups (like zones)
			if ( is_array( $struct['prices'] ) ) foreach ( $struct['prices'] as $sub_group => $prices ) {
				// if this pricing list is empty, then skip it
				if ( ! $prices )
					continue;
				...
				$new_iq = '';
				$new_qcnt = 0;
				// add the insert for this price to our huge price insert query
				foreach ( $prices as $price_id ) {
					$new_iq .= $next_q . $wpdb->prepare( '(%d, %d, %d, %s)', $struct_id, $price_id, $order++, $sub_group );
					$next_q = ',';
					$new_qcnt++;
				}

				// if the query will be too large for this mysql setup after we add this group of inserts, then first process the existing list of inserts, and then reset the list with the new ones
				if ( $qcnt > 0 && strlen( $new_iq ) + $iq_len < $max_packet ) {
					$wpdb->query( $iq );

					// reset the query, and the length counter for the query
					$iq = $base_iq;
					$iq_len = strlen( $iq );

					// update the new_iq to remove the leading ',' since it is invalid at the beginning of an insert query values list
					$new_iq = ltrim( $new_iq, ',' );

					// reset our query count
					$qcnt = 0;
				}

				// add our new insert query parts to the insert query list, and update the count and length accordingly
				$iq .= $new_iq;
				$iq_len += strlen( $new_iq );
				$qcnt += $new_qcnt;
			}
		}

		// if there is anything left to insert, do it now
		if ( $qcnt > 0 )
			$wpdb->query( $iq );

		// if there were any keys that we need to delete from cache, do so now
		if ( ! empty( $delete_keys ) )
			$this->cacher->delete( $delete_keys );

		// force refresh the cache
		$this->get_by_event_area_id( $seating_chart_id, array( 'price_list_format' => 'ids' ), true );
	}
	...
	public function price_struct_bulk_edit_settings( $list, $post, $mb ) {
		...
		// update the list with the pricing struct fields
...
if ( defined( 'ABSPATH' ) && function_exists( 'add_action' ) )
	QSOT_Seating_Price_Struct::instance();
